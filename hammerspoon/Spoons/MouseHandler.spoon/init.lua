   ...
   self.mouseTap = hs.eventtap.new(
      ...
      function(e)
         ...
         if m == "mouseUp" then
            ...
            local clickFunc = nil
            -- verificação para evitar acesso a índices de tabelas nulas ou vazias
            if click and type(click) == "table" and click[1] then
               clickFunc = click[1]
            elseif config[bName] and config[bName]['click'] and type(config[bName]['click']) == "table" and config[bName]['click'][1] then
               clickFunc = config[bName]['click'][1]
            end

            if this.debug then
               hs.printf("mouse up detected frontApp = %s %s b = %s %s %s", frontApp, bName, inspect(click),
                  inspect(this.longPressTasks),
                  this.debug)
            end
            hs.printf("click func found %s", clickFunc)
            if clickFunc then
